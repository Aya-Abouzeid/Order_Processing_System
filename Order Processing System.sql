drop schema OrderProcessingSystem;
	...
	FOR EACH ROW BEGIN
		...
		IF NEW.Stock<NEW.Threshold THEN
            ...
            SET @ISBN = NEW.ISBN;
             ...
             if( exists ( select * from book_orders where isbn = new.isbn)) then
				update book_orders set quantity = quantity + @addQuantity;
                
			else
            INSERT INTO BOOK_ORDERS VALUES (@ISBN,@addQuantity);
            end if;
